...
const tweet_exists_in_TimelinesTable = async (userId, tweetId) => {
  ...
  console.log(
    `looking for tweet [${tweetId}] for user [${tweetId}] in table [${process.env.TIMELINES_TABLE}]`
  );
  const resp = await DynamoDB.get({
...
const retweet_exists_in_RetweetsTable = async (userId, tweetId) => {
  ...
  console.log(
    `looking for retweet of [${tweetId}] for user [${tweetId}] in table [${process.env.RETWEETS_TABLE}]`
  );
  const resp = await DynamoDB.get({
